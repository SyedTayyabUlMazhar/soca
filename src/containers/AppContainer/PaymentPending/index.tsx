import {pendingPayments} from '@Api/App';
...
const PaymentPending = ({route}) => {
  ...
  const {mutate: pendingMutate} = useMutation(pendingPayments, {
    onSuccess: response => {
      setData(response);
    },
  ...
  useEffect(() => {
      const payload = { parentId: email[0] };
      pendingMutate(payload);
  }, []);

  const renderItem = ({item}) => (
    <View style={styles.row}>
      <Text style={styles.cell}>{item?.title}</Text>
      <Text style={[styles.cell, {marginLeft: Metrics.scale(70)}]}>
        {item?.best}
      </Text>

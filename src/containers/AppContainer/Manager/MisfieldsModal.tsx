...
interface ICustomModal {
  ...
  const MisfieldsModal = ({
    ...
    const [count, setCount] = useState(selectedPlayer ? selectedPlayer.misfields : 0);
  const {updateFieldingSessionMutate}=useManagerContainer()
    ...
    useEffect(() => {
      if (selectedPlayer) {
          setCount(selectedPlayer.misfields);
      }
  }, [selectedPlayer]);
    const increment = () => {
